...
-- 1) 컬럼 추가 (nullable로 우선 → backfill → NOT NULL)
ALTER TABLE catalog_products ADD COLUMN IF NOT EXISTS channel_account text;

-- 2) 기존 rows(모두 카페24 sync)를 브랜드의 cafe24 channel_account로 backfill
...
-- 4) 기존 UNIQUE (brand_id, product_no) → 새 UNIQUE (brand_id, channel_account, product_no)
