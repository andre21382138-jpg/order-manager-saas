-- Plan 14 Phase 12 fix: get_category_ad_group_details의 mapped_count 계산 수정
-- 캠페인의 mapped_count는 전체 카테고리 매핑 기준이어야 결산조회 표와 일치.
...
RETURNS TABLE (
  ...
  campaigns AS (
    ...
    FROM ad_units WHERE brand_id = p_brand_id AND level = 'campaign'
  ),
  -- 전체 카테고리 기준 캠페인별 매핑 광고그룹 수 (결산조회 표와 동일 기준)
  campaign_total_mapping_counts AS (
    SELECT
      ag.campaign_unit_id,
      COUNT(DISTINCT agcm.ad_group_id) AS mapped_count
    FROM ad_group_category_mappings agcm
    JOIN ag_meta ag ON ag.ad_group_id = agcm.ad_group_id
  ...
  FROM mapped_in_category mic
  JOIN ag_meta ag ON ag.ad_group_id = mic.ad_group_id
  LEFT JOIN campaigns c ON c.id = ag.campaign_unit_id
  LEFT JOIN campaign_total_mapping_counts cmc ON cmc.campaign_unit_id = ag.campaign_unit_id
  LEFT JOIN campaign_stats cs ON cs.campaign_unit_id = ag.campaign_unit_id
  GROUP BY mic.ad_group_id
  ORDER BY 5 DESC;
$$;
