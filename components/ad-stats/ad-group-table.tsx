...
import type { AdGroupRow, CampaignRow } from '@/lib/queries/ad-stats'
...
function derived(r: AdGroupRow): Record<SortKey, number> {
  return {
    cost: r.cost,
  ...
  const filtered = useMemo(() => {
    ...
    return data.filter(
      (r) =>
        ...
        (q === '' || r.ad_group_name.toLowerCase().includes(q))
    )
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {sorted.map((r) => {
                const d = derived(r)
                return (
                  <tr key={r.ad_group_id} className="border-b">
                    <td className="py-2 pr-4 font-medium">{r.ad_group_name}</td>
