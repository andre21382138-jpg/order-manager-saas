          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b text-left text-muted-foreground">
                ...
                <th className="py-2 pr-4 text-right">원가비중</th>
                <th className="py-2 pr-4 text-right">광고비</th>
                <th className="py-2 pr-4 text-right">광고비중</th>
              </tr>
            </thead>
            <tbody>
              {data.length === 0 && (
              ...
              {data.map((r, i) => (
                <tr key={`${r.catalogProductId ?? r.productName}-${i}`} className="border-b">
                  ...
                  <td className="py-2 pr-4 text-right">{fmtCount(r.qty)}개</td>
                  <td className="py-2 pr-4 text-right">{fmtWon(r.amount)}</td>
                  <td className="py-2 pr-4 text-right">{fmtPct(r.costRate)}</td>
                  <td className="py-2 pr-4 text-right">{fmtWon(r.adCost)}</td>
                  <td className="py-2 pr-4 text-right">{fmtPct(r.adRate)}</td>
                </tr>
              ))}
            </tbody>
          </table>
